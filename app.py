...
contractors_file = 'Contractors.csv'

# Load data
try:
   tasks = pd.read_csv(tasks_file)
except FileNotFoundError:
   tasks = pd.DataFrame(columns=["Task", "Date", "Contractor",
"Notes", "Status"])
...
task_contractor = st.sidebar.selectbox("Contractor",
contractors['Name'].tolist())
task_notes = st.sidebar.text_area("Notes")

if st.sidebar.button("Add Task"):
   new_task = pd.DataFrame([[task_name, task_date, task_contractor,
task_notes, "Upcoming"]],
                           columns=["Task", "Date", "Contractor",
"Notes", "Status"])
...
with open('Maintenance_tips.md') as f:
   tips = f.read()
st.markdown(tips)
